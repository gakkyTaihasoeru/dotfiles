...
- `~/.config/chezmoi/chezmoi.toml` の `sourceDir` を本リポジトリに向ける
- `chezmoi apply` を実行（後述の run_once script もここで走る）
...
├── .github/workflows/ci.yml         # CI (shellcheck / shfmt / chezmoi dry-run / JSON / TOML / YAML / py syntax)
├── bin/
...
└── home/                            # ← chezmoi source root
    ...
    │   ├── rules/
    │   └── hooks/
    │       ├── executable_secret-scan.py
    │       └── executable_sensitive-read.py
    ├── dot_gemini/                  # → ~/.gemini/
    │   ├── GEMINI.md
    │   ├── settings.json
